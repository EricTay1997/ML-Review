...
- Convolutions
  ...
  - Additional Details
    - Convolutions, without padding and with stride 1, transform a tensor of shape $`N \times C_{in} \times H \times W`$ into $`N \times C_{out} \times (H-k+1) \times (W-k+1)`$.
    ...
    - With padding and stride, we get the following output shape
      - $`\displaystyle H_{out} = \left\lfloor\frac{H_{in}  + 2 \times \text{padding} - \text{dilation} \times (\text{kernel\_size} - 1) - 1}{\text{stride}} + 1\right\rfloor`$
- Pooling
