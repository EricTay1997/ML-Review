...
- ![activations.png](images/activations.png)[Source](https://medium.com/the-modern-scientist/an-overview-of-activation-functions-in-deep-learning-97a85ac00460)
- Swish
  - $\mathrm{Swish}(x) = \frac{x}{1+e^{-\beta x}}$
- GeLU
  - $\mathrm{GELU}(x) = xP(X \leq x)$ for $X \sim \mathcal{N}(0,1)$
  - $\mathrm{GELU}(x) \approx 0.5 \cdot x \cdot\left(1+\tanh \left[\sqrt{\frac{2}{\pi}} \cdot\left(x+0.044715 \cdot x^3\right)\right]\right)$
  - ![gelu.png](images/gelu.png)[Source](https://paperswithcode.com/method/gelu)
