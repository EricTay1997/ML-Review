...
- Normalization
  - Batch Normalization
    ...
    - Since batch statistics can have high variance especially with small batches,
      - In training, we normalize with the current batch's statistics, and separately maintain a running mean and variance (as an EMA) for later use in eval.
...
- PyTorch Buffers
  ...
  - PyTorch buffers are tensor attributes associated with a PyTorch module or model similar to parameters, but unlike parameters, buffers receive no gradients and are not updated by the optimizer (module code may still mutate them during training — e.g. BatchNorm's running stats above).
