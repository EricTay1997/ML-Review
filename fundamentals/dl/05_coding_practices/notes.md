...
def f2(a, b):
    print(f"{a}, {b}")
```
    - The decorator is applied once, when `def` executes: `f1 = time_function(f1)`. The decorator never sees call-site arguments — a later call `f2(3, 4)` invokes the returned `wrapper(3, 4)`, which forwards the arguments to the original `f2`.
    - Note the decorator must *return a wrapper function*; a version that calls `f()` immediately and returns its result would run the function at definition time and rebind the name to the result.
    - Common decorators
      - Python
        - `@classmethod`
