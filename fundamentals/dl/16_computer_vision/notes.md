...
- Classification:
  - [Lippe's implementation](https://github.com/phlippe/uvadlc_notebooks/blob/master/docs/tutorial_notebooks/tutorial15/Vision_Transformer.ipynb):
    ...
    - Flattened patches are first linearly projected to embedding dim $`D`$; the learned positional encodings then have $`(L + 1) \times D`$ parameters (+1 for the CLS token). (Under the no-projection simplification where the embedding is the raw $`C \cdot p_H \cdot p_W`$ vector, rows compare patches and columns compare within-patch pixels.) 
...
- One-Stage Detector
  ...
  - YOLOv2
    - Builds on YOLO
      ...
      - Adds a passthrough layer that reshapes and concatenates fine-grained higher-resolution features with the final features (skip *concatenation* — true residual connections arrive with Darknet-53 in YOLOv3)
