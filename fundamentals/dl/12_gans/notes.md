...
- Improved Training 
  ...
  - One-sided label smoothing
    - Soften only the real-sample target (1 → 0.9); leave fake-sample targets at exactly 0. (Smoothing the fake side too puts a $`p_{model}`$ term in the optimal discriminator's numerator, reinforcing the generator's existing errors — Salimans et al. 2016, §3.4.) 
