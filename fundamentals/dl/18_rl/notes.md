...
- Training: How do we learn this policy?
  ...
  - Deep Q-Learning
    ...
    - A few tricks we use to stabilize training:
      ...
      - Double DQN 
        - Double DQN keeps the lagging target network, but decouples action *selection* from action *evaluation* to reduce overestimation bias.
        - Select the action with the online (trained) network, $`a^* = \arg\max_a Q_{online}(s_1, a)`$, then evaluate it with the target network: target $`= r + \gamma Q_{target}(s_1, a^*)`$.