...
- Continuity/transport equation:
  - $`\frac{dp_\theta(\mathbf{x}_t)}{dt} = -\mathrm{div}(p_\theta(\mathbf{x}_t)u_t(\mathbf{x}_t, \theta))`$
- $\frac{d\log p_\theta(\mathbf{x}_t)}{dt} = -\mathrm{div}(u_t(\mathbf{x}_t, \theta))$
- $`\log p_\theta(\mathbf{x}) = \log p_0(\mathbf{x}_0)- \int_{t=0}^1\mathrm{div}(u_t(\mathbf{x}_t, \theta))dt`$
