...
- Continuity/transport equation:
  - $`\displaystyle \frac{dp_\theta(\mathbf{x}_t)}{dt} = -\mathrm{div}(p_\theta(\mathbf{x}_t)u_t(\mathbf{x}_t, \theta))`$
- $\frac{d\log p_\theta(\mathbf{x}_t)}{dt} = -\mathrm{div}(u_t(\mathbf{x}_t, \theta))$
- $`\displaystyle \log p_\theta(\mathbf{x}) = \log p_0(\mathbf{x}_0)- \int_{t=0}^1\mathrm{div}(u_t(\mathbf{x}_t, \theta))dt`$
...
- Now we simply train our neural network $v_t\left(\mathbf{x}_{\mathrm{t}}, \theta\right)$ to match this conditional vector field. 
  - $`\displaystyle \mathbb{E}_{t, q(\mathbf{z}), p_t\left(\mathbf{x}_t \mid \mathbf{z}\right)}\left[\left\|v_t\left(\mathbf{x}_{\mathrm{t}}, \theta\right)-u_t\left(\mathbf{x}_{\mathrm{t}} \mid \mathbf{z}\right)\right\|_2^2\right]`$
...
- $\mathrm{bpd} = \frac{\log_2(e)\mathrm{nll}}{\prod d_i}$ gives us a metric which we can compare for different image resolutions
