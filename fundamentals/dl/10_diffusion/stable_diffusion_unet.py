# Taken from https://github.com/labmlai/annotated_deep_learning_paper_implementations/blob/master/labml_nn/diffusion/stable_diffusion/model/unet_attention.py
...
# This treats each pixel as a token, and the channel as the embedding
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        :param channels: is the base channel count for the model, num channels for "top" row
        ...
        # for example, convolution only accepts the feature map,
        # residual blocks accept the feature map and time embedding,
        # attention blocks accept the feature map and conditioning
        # `TimestepEmbedSequential` calls them accordingly.
        self.input_blocks.append(TimestepEmbedSequential(
            nn.Conv2d(in_channels, channels, 3, padding=1)))
        # Number of channels at each block in the input half of U-Net
        # This is needed for the 2nd half, because we need to know how many channels we're concatenating
        input_block_channels = [channels] # 320, 320, 320, 640, 640, ...
        ...
        for i in range(levels):
            for _ in range(n_res_blocks):
                ...
                # Add transformer on certain levels
    ...
    def time_step_sinusoidal_embedding(self, time_steps: torch.Tensor, max_period: int = 10000):
        """
    ...
    def forward(self, x: torch.Tensor, time_steps: torch.Tensor, cond: torch.Tensor):
        ...
        t_emb = self.time_step_sinusoidal_embedding(time_steps)
...
def _test_time_embeddings():
    ...
    te = m.time_step_sinusoidal_embedding(torch.arange(0, 1000))
