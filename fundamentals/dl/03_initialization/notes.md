...
* Xavier initialization: $\mathrm{Var}(w_{ij}) = \frac{2}{d_x + d_y}$
  * Suppose now we want two conditions 
    * The mean of the activations should be zero 
    * The variance of the activations should stay the same across every layer
  * We have the following relation:
    * $`\mathrm{Var}(y_i) = \mathrm{Var}(\sum_j w_{ij}x_j) = \sigma_x^2 = \sigma_x^2 d_{x} \mathrm{Var}(w_{ij})`$, which gives
    * $`\mathrm{Var}(w_{ij}) = \frac{1}{d_{x}}`$
...
* Kaiming initialization: $\mathrm{Var}(w_{ij}) = \frac{2}{d_x}$
  * For ReLU, we have
    * $`\mathrm{Var}(w_{ij}x_j) = \mathbb{E}(w_{ij}^2)\mathbb{E}(x_{j}^2) - \mathbb{E}(w_{ij})^2\mathbb{E}(x_{j})^2 = \mathrm{Var}(w_{ij})\mathbb{E}(x_{j}^2)`$
    * Now taking $`x_j`$ as the post-ReLU output of the previous layer, we get that $`\mathbb{E}(x^2) = \frac{1}{2}\mathrm{Var}(\tilde{y}),`$ where $`\tilde{y}`$ is the pre-ReLU activations of the previous layer. 
...
* PyTorch
  * PyTorch ([per mrTsjolder](https://discuss.pytorch.org/t/how-are-layer-weights-and-biases-initialized-by-default/13073/33)) initializes linear layers with $\mathrm{Var}(w_{ij}) = \frac{1}{3d_x}$