...
- The predicted probability is a weighted sum of labels of support set samples:
  ...
  - ProtoNet:
    ...
    - $`p(y=c\mid\mathbf{x}) = \mathrm{softmax}(-d_\varphi(f_\theta(\mathbf{x}), \mathbf{v}_c))`$, where we typically use the squared euclidean distance for our distance metric $`d_\varphi`$.
