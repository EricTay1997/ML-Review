...
- Some audio encoders — e.g. the spectrogram VAEs used in latent-diffusion pipelines like AudioLDM — convert audio into a mel-spectrogram and process it like an image. (Note: [SoundStream](https://arxiv.org/pdf/2107.03312) itself encodes the raw waveform with 1D convolutions; mel-spectrograms only appear in its losses/discriminator, so this critique applies to the spectrogram-based family, not SoundStream.) 
