...
- Eigenvectors and eigenvalues
  ...
  - Eigendecomposition
    ...
    - If all eigenvalues of a real _symmetric_ matrix are positive, the matrix is positive definite. ($`\forall`$ $`\mathbf{x} \neq \mathbf{0}, \mathbf{x^{\top}Ax}>0`$. Proof: We can express $`\mathbf{x}`$ in the orthonormal eigenbasis and $`\mathbf{v^{\top}Av}=\lambda>0`$ for each eigenvector. Symmetry matters: a non-symmetric matrix can have positive eigenvalues yet an indefinite quadratic form.)
...
- Trace
  ...
  - When the product is a square matrix, $\mathrm{Tr}\left(\prod_{i=1}^n \mathbf{F}^{(i)}\right)=\mathrm{Tr}\left(\mathbf{F}^{(n)} \prod_{i=1}^{n-1} \mathbf{F}^{(i)}\right)$
...
- Hessian and the nature of a stationary point
  - For twice continuously differentiable $f$: at a local minimum, necessarily $\nabla f(\mathbf{x}) = \mathbf{0}$ and $\nabla^2f(\mathbf{x})$ is positive semi-definite (at the point). Conversely, $\nabla f(\mathbf{x}) = \mathbf{0}$ with $\nabla^2f(\mathbf{x})$ positive definite (or PSD on a neighborhood) is sufficient. Neither direction is an iff.
...
- Integrals:
  ...
  - Change of variables: $`\int_{\phi(U)} f(\mathbf{x}) d \mathbf{x}=\int_U f(\phi(\mathbf{x}))|\mathrm{det}(D \phi(\mathbf{x}))| d \mathbf{x}`$, where $`D \pmb{\phi}=\left[\begin{array}{ccc}\frac{\partial \phi_1}{\partial x_1} & \cdots & \frac{\partial \phi_1}{\partial x_n} \\ \vdots & \ddots & \vdots \\ \frac{\partial \phi_n}{\partial x_1} & \cdots & \frac{\partial \phi_n}{\partial x_n}\end{array}\right]`$
...
- Simple case: Suppose we want to find $\hat{\mathbf{x}}$ that minimizes $f(\mathbf{x})$ subject to $g(\mathbf{x}) = c$
  ...
  - An alternative strategy for satisfying constraints is projections. Note, however, that the constrained optimum is generally _not_ the Euclidean projection of the unconstrained optimum $`\mathbf{x}^*`$ onto $`g(\mathbf{x}) = c`$ — that holds only when the level sets of $`f`$ are spheres centered at $`\mathbf{x}^*`$ (for a quadratic with Hessian $`\mathbf{H}`$, it is the projection in the $`\mathbf{H}`$-norm).
