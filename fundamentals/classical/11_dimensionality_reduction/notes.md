  ...
  - Proof:
    - $`\arg\max _{\mathbf{u}:\|\mathbf{u}\|=1} \mathbf{u}^{\top} \mathbf{S u}`$ gives us the direction that projecting $`\mathbf{X}`$ onto would maximize variance. ($`\mathbf{S} = \frac{1}{n-1}\mathbf{Z^{\top}Z}`$ is our sample covariance matrix, where $`\mathbf{Z}`$ is $`\mathbf{X}`$ with demeaned columns)
      ...
      - Note that $`\mathbf{Xu} \in \mathbb{R}^n`$ is this projection. 
        - Why? Each entry is the dot product of a row (sample) with $`\mathbf{u}`$, and the dot product with the unit vector $`\mathbf{u}`$ is the scalar projection $`\|\mathbf{x}\|\cos\theta`$ — the sample is the hypotenuse and its projection onto $`\mathbf{u}`$ is the adjacent side. 
      - Now $`\mathrm{Var}(\mathbf{Xu}) = \mathbf{uSu}`$.
...
- t-SNE
  ...
  - Algorithm:
    ...
    - Calculate $`p_{ij} = \frac{p_{j \mid i} + p_{i \mid j}}{2n}`$ (the $`n`$ makes $`\sum_{ij} p_{ij} = 1`$, a valid distribution for the KL below)
