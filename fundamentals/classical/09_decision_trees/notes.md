...
- Computational Complexity
  - With $n$ features and $m$ samples, training is generally $O(nm\log m)$ (compare $n$ features across $m$ samples per level, over $O(\log m)$ levels); inference traverses one root-to-leaf path, $O(\log m)$ for a balanced tree.
...
- Cost Function
  ...
  - $\mathcal{L}(k, t_k) = \frac{n_{left}}{n}H_{left} + \frac{n_{right}}{n}H_{right}$, where $n$ denotes the number of samples and $H$ is a measure of entropy.
...
- ![regression_trees.png](images/regression_trees.png)[Source](https://link.springer.com/chapter/10.1007/978-3-031-49529-8_8)
- CART
