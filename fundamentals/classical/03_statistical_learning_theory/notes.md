...
* MSE & Bias-Variance Tradeoff
  * $\mathrm{MSE}= \mathbb{E}\left[\left(\hat{\theta}_m-\theta\right)^2\right] \\ =\mathrm{Bias}\left(\hat{\theta}_m\right)^2+\mathrm{Var}\left(\hat{\theta}_m\right)$
  ...
  * Note a similar decomposition of the data generating process $y = f(x) + \epsilon$, $\mathrm{Var}(\epsilon) = \sigma^2$
    * $`\mathrm{MSE} = \mathbb{E}[(y-\hat{y})^2] = \mathrm{Bias}(\hat{y})^2+\mathrm{Var}(\hat{y})+\sigma^2`$, where we term $`\sigma^2`$ as the irreducible error.
...
* Consistency
  * $\mathrm{lim}_{n \rightarrow \infty} \hat{\theta}_n=\theta$
  * Biased but consistent: 
    ...
    * This estimator is consistent because $`\mathrm{Var}(\hat{\sigma}^2) = \frac{2\sigma^4 (n-1)}{n^2}`$
      * Proof: Using [Cochran's theorem](https://en.wikipedia.org/wiki/Cochran%27s_theorem#Sample_mean_and_sample_variance), we have that $`\frac1n\sum^n (x_i - \bar{x})^2 \sim \frac{\sigma^2}{n}\chi^2_{n-1},`$ and $`\mathrm{Var}[\chi^2_{n-1}] = 2(n-1)`$
...
* Efficiency (let's switch to scalars for simplicity)
  ...
  * The Cramér–Rao lower bound for the scalar unbiased case is then given by $\mathrm{Var}(\hat{\theta})\geq\frac{1}{I(\theta)}$.
  ...
  * Now, **Efficiency** for an unbiased estimator is then defined as $e(\hat{\theta}) = \frac{I(\theta)^{-1}}{\mathrm{Var}(\hat{\theta})}$. This is bounded above by 1 and it gives a measure of how precise the estimator is (relative to the theoretical maximum precision).
