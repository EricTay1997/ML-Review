...
- Experiments
  ...
  - Natural/quasi-experiments: Assignment mechanism is known but not controlled by the researcher, and plausibly "as-if random" (determined by nature or policy). 
- Inference
...
- Extending past linear models
  ...
  - [Double Machine Learning](https://matheusfacure.github.io/python-causality-handbook/22-Debiased-Orthogonal-Machine-Learning.html) provides a framework for doing so, and proceeds in 3 steps:
    ...
    - For the ATE, regress $`\tilde{Y}`$ on $`\tilde{T}`$ — the slope is the effect. For CATEs, train $`m_3(X)`$ against the target $`\frac{\tilde{Y}}{\tilde{T}}`$ **weighted by** $`\tilde{T}^2`$ (the weights matter; unweighted, the noisy small-$`\tilde{T}`$ targets dominate).
...
- To handle unmeasured confounding, we use instrumental variables (IV), which influence treatment assignment but is independent of unmeasured confounders, and has no direct effect on the outcome except through its effect on treatment.
  ...
  - ![iv.png](images/iv.png)[Source](https://link.springer.com/referenceworkentry/10.1007/978-1-4939-6704-9_7-1)
...
- Noncompliance
  ...
  - Now, however, if we are interested in the effect of the treatment $Z_i$ itself on $Y_i$, the ratio below identifies the **LATE/CACE** — the average effect among compliers — not the ATE of forcing everyone (the two coincide only under constant-effect assumptions):
