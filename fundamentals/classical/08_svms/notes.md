...
- The prediction for a point $\mathbf{x} \in \mathbb{R}^p$ is given by $\mathrm{sign}(\pmb{\lambda}^{\top}\mathbf{x}+\lambda_0)$
  - Geometrically, $\pmb{\lambda}$ and $\lambda_0$ define a hyperplane in $\mathbb{R}^{p}$ (the parameter vector has $p+1$ entries), where points on different sides are classified as 1 and -1.
  - See more in [Understanding MLPs](../../dl/26_personal_projects/understanding_mlps/notes.md)
- Support Vectors
...
- Jumping ahead, we have the following solution for $\hat{\pmb\alpha}$ and $\hat{\lambda_0}$:
  - $`\hat{\pmb\alpha} = \mathrm{argmax}_{\pmb\alpha}\mathcal{L}(\pmb{\alpha}), \mathcal{L}(\pmb{\alpha})=\sum_{i=1}^n \alpha_i-\frac{1}{2} \sum_{i, k} \alpha_i \alpha_k y_i y_k \mathbf{x}_i^T \mathbf{x}_k \quad`$ s.t. $`\quad\left\{\begin{array}{l}\alpha_i \geq 0 \quad i=1 \ldots n \\ \sum_{i=1}^n \alpha_i y_i=0\end{array}\right.`$
...
- Proof:
  - We want to maximize the margin $\mathrm{min}_i \left( y_i(\pmb\lambda^{\top}\mathbf{x}+\lambda_0) \right)$
...
- Jumping ahead, we have the following solution:
  - $`\hat{\pmb\alpha} = \mathrm{argmax}_{\pmb\alpha}\mathcal{L}(\pmb{\alpha}), \mathcal{L}(\pmb{\alpha})= \sum_{i=1}^n \alpha_i-\frac{1}{2} \sum_{i, k=1}^n \alpha_i \alpha_k y_i y_k \mathbf{x}_i^T \mathbf{x}_k \quad`$ s.t. $`\quad\left\{\begin{array}{l}0 \leq \alpha_i \leq C \quad i=1 \ldots n \\ \sum_{i=1}^n \alpha_i y_i=0\end{array}\right.`$
  - $\hat{\pmb{\lambda}}=\sum_{i=1}^n \hat{\alpha}_i y_i \mathbf{x}_i$
...
- Kernel Regression
  - Kernel regression uses a kernel as a weighting function: $\mathrm{pred}(x) = \frac{\sum_i y_ik(x, x_i)}{\sum_i k(x, x_i)}$
  ...
  - ![higher_dim.png](images/higher_dim.png)[Source](https://behesht.medium.com/support-vector-machies-part1-classification-fc1cc382b8a4)
...
- Prediction on a new point $\mathbf{x}$ = $\mathrm{sign}(f(\mathbf{x})),$ where $f(\mathbf{x})= \hat{\lambda_0} + \sum_i^n\alpha_iy_ik(\mathbf{x_i, x})$
  - Why is this useful?
...
- In the "soft margin" case, we add two slack variables such that:
  - $y_i - (\mathbf{B^{\top}x}_i + B_0) \leq \epsilon + \xi_i$
  - $(\mathbf{B^{\top}x}_i + B_0) - y_i \leq \epsilon + \xi_i^*$
