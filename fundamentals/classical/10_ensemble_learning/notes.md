...
- AdaBoost
  ...
  - $\hat{y}(\mathbf{x}) = \arg\max_k \sum_{j = 1, \hat{y}_j(\mathbf{x}) = k}^J \alpha_j$ (Output the class with the highest weighted vote), where $\hat{y}_j(\mathbf{x})$ represents the prediction of the $j^{th}$ classifier.
  - Algorithm
...
- Gradient Boosted Trees
  ...
  - LightGBM
    - Histogram-based splits (bins feature values): Faster training speed
...
- Now replace the linear terms with per-feature smooth functions: $g(\mu) = b_0 + f_1(x_1) + \ldots + f_p(x_p), \quad \mathbf{x} = (x_1, \ldots, x_p)^\top$. 
- One example is spline smoothing, where each $f_j$ is a spline basis expansion of its own feature $x_j$ (each feature gets its own flexible curve, and the curves add).
