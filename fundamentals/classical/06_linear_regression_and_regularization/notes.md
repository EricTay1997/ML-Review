...
- Assumptions
  ...
  - Homoscedasticity: $\mathrm{Var}(\epsilon_i) = \sigma^2$
...
- t-stats
  - Since $\mathbf{\hat{B}} = (\mathbf{X}^{\top}\mathbf{X})^{-1}\mathbf{X}^{\top}\mathbf{y}$ is an affine transformation of $\mathbf{y} \sim \mathcal{N}(\mathbf{XB}, \sigma^2\mathbf{I})$, 
    - $`\mathbf{\hat{B} \mid \mathbf{B}, \sigma^2}\sim\mathcal{N}(\mathbf{B}, \sigma^2(\mathbf{X^\top X})^{-1})`$ (using $`\mathrm{Var}(\mathbf{Ax}) = \mathbf{A}(\mathrm{Var(\mathbf{x})})\mathbf{A}^{\top}`$)
...
* Model Selection
  ...
  * Partial F-test for nested models (the Chow test is the structural-break special case, testing whether coefficients differ across two subsamples)
...
* Homoskedasticity 
  ...
  * Remedies: Robust Standard Errors
    * White's Estimator (heteroskedastic, but still diagonal)
      * $`\hat{\mathrm{Var}(\hat{\beta})} =\frac{\mathbf{1}}{\mathbf{n}}\left[\frac{\mathbf{1}}{\mathbf{n}}\left(\mathbf{X}^{\prime} \mathbf{X}\right)\right]^{-\mathbf{1}}\left[\frac{\mathbf{1}}{\mathbf{n}} \mathbf{X}^{\prime} \hat{\pmb{\Omega}} \mathbf{X}\right]\left[\frac{\mathbf{1}}{\mathbf{n}}\left(\mathbf{X}^{\prime} \mathbf{X}\right)\right]^{-\mathbf{1}}`$, $`\hat{\pmb{\Omega}} =\mathrm{diag}\left(\hat{\mathrm{u}}_1^2, \hat{\mathrm{u}}_2^2, \cdots, \hat{\mathrm{u}}_{\mathrm{n}}^2\right)`$ from OLS errors.
...
* Multicollinearity 
  * Detection
    * VIF = $`\frac{\mathrm{Var}(B_j)_{full}}{\mathrm{Var}(B_j)_{reduced}}`$, where the reduced model only has parameter $`j`$.
...
- Instead of minimizing $\sum_i (y_i - \hat{y_i})^2$, we instead minimize $\sum_i w_i(y_i - \hat{y_i})^2$.
  - Looking at the likelihood function, we can think of this as having heteroscedastic error terms, with $\mathrm{Var}(\epsilon_i) = \frac{\sigma^2}{w_i}$
...
- Ridge Regression
  ...
  - $`\mathrm{MSE}(\hat{\mathbf{B}}_{MLE}) = E[||\hat{\mathbf{B}}_{MLE}-\mathbf{B}||^2] = \sigma^2\mathrm{tr}\left[\left(\mathbf{X}^T \mathbf{X}\right)^{-1}\right] =\sigma^2 \sum_{j=1}^p \lambda_j^{-1}`$
...
- Lasso Regression
  - $\mathcal{L}(\mathbf{B}) = ||\mathbf{Y} - \mathbf{XB}||^2_2 + \lambda||\mathbf{B}||_1$
  - Optimization
...
- Elastic Net
  - $\mathcal{L}(\mathbf{B}) = ||\mathbf{Y} - \mathbf{XB}||^2_2 + \lambda_1||\mathbf{B}||^2_2 + \lambda_2||\mathbf{B}||_1$
