...
* Expectations and Variance
  ...
  * Random Vector $\mathbf{x} = (X_1, \ldots, X_p)^{\top}$
    ...
    * $`\pmb{\Sigma} \stackrel{\text { def }}{=} \mathrm{Cov}_{\mathbf{x} \sim p}[\mathbf{x}]=E_{\mathbf{x} \sim p}\left[(\mathbf{x}-\pmb{\mu})(\mathbf{x}-\pmb{\mu})^{\top}\right]`$ is the covariance matrix. 
      * $`\Sigma_{ij} = \mathrm{Cov}(X_i, X_j)`$ 
      * This is closely related to the correlation matrix, where $`\Sigma_{ij} = \frac{\mathrm{Cov}(X_i, X_j)}{\sigma(X_i)\sigma(X_j)}`$
      * $`\pmb{\Sigma}`$ is positive semi-definite: 
        * $`\mathbf{v^{\top}}\pmb{\Sigma}\mathbf{v} = \sum_i\sum_j v_iv_jCov(X_i, X_j)`$, which we recognize to be the formula of $`\mathrm{Var}(\mathbf{v^{\top}x}) \geq 0.`$
  ...
  * These formulae are relatively easy to apply given a data generating process with specified parameters. In practice, we usually estimate parameters from data:
    * Suppose $`X_i`$ are iid and $`E[X] = \mu`$ and $`\mathrm{Var}(X) = \sigma^2`$
    ...
    * **Sample Covariance Matrix**
      ...
      * The sample covariance matrix $`\mathbf{S} \in \mathbb{R}^{p \times p}`$ is an unbiased estimate of $`\pmb{\Sigma} = \mathrm{Cov}(\mathbf{x})`$. I.e. $`E[S_{jk}] = \mathrm{Cov}(X_j, X_k)`$.
        * Concretely, 
          ...
          * Note that this extends the unbiased estimator for population variance above to the unbiased estimator of $`\mathrm{Cov}(X_j, X_k)`$.
