...
- Distributing Model Training: `tf.distribute` strategies or Horovod (YARN is Hadoop's cluster resource manager; TonY is the project that runs TF jobs on YARN)
...
- Need to know
  - Kafka is biased toward consistency/durability rather than availability: with unclean leader election disabled (default), a partition that loses all in-sync replicas goes offline instead of serving potentially inconsistent data; `acks=all` + `min.insync.replicas` refuses writes rather than losing them
...
- Good to know
  ...
  - A **consumer group** is a group of consumers. A consumer group can subscribe to one or more topics. Each event is processed by only one consumer in a group.
  - Fault tolerance / Durability
    - Offset management: Consumers commit their offsets to Kafka after they process a message. When a consumer restarts, it reads its last committed offset from Kafka and resumes processing from there — this gives at-least-once delivery (no messages missed, but duplicates are possible on a crash between processing and commit); exactly-once needs idempotent processing or Kafka transactions.
