...
- [Distributed Training](../../../llms/performance/parallelism.md)
- Sampling from predicted probabilities is generally (depending on use case) desirable — but it is only meaningful if the model is well calibrated (calibration is a property to be obtained via e.g. Platt scaling or isotonic regression; sampling doesn't create it). 
