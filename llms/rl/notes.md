...
- **But it turns out that $`k_3`$ as a loss is not principled**. Its gradient is a first-order approximation of the true reverse-KL gradient, biased and asymmetric in the tails; the principled choices are $`k_1`$ in the reward or $`k_2`$ as a loss. Worked out in [KL Divergence](kl_divergence.md).
  - The original objective has a second gap: the importance ratio multiplies only the advantage term, so with more than one gradient step per rollout batch, the KL head is off-policy and uncorrected (moot for DeepSeekMath itself, which did a single update per batch). DeepSeek fixed both in [V3.2](https://arxiv.org/abs/2512.02556) (eq. 7) by multiplying the $`k_3`$ term by $`\pi_\theta/\pi_{\mathrm{old}}`$. This change also repairs the gradient, because $`\nabla_\theta(\rho\,k_3) = \rho\,(-\log\delta)\,\nabla_\theta\log\pi_\theta`$: the ratio's score term contributes $`k_3`$, the direct term contributes $`1-\delta`$, and they sum to $`k_1`$ (proof in [KL Divergence](kl_divergence.md)). TRL's GRPOTrainer does this by default now (`use_bias_correction_kl`); verl's `k3+` reaches the same gradient with a straight-through $`k_2`$.
...
- **"Token-level" loss: one mean over every token in the prompt's group.** GRPO averages per sequence then per group ($`\frac1G \sum_i \frac{1}{|o_i|}\sum_t`$), so every *sample* has equal weight and tokens in long responses count less. For a positive advantage, shorter responses get a larger per-token push (prefer short correct answers); for a negative advantage, longer responses get a *smaller* per-token penalty (wrong answers drift longer). DAPO instead averages over all tokens in the prompt group ($`\frac{1}{\sum_i |o_i|}\sum_i\sum_t`$): long correct responses aren't under-rewarded, and long rambling ones aren't under-penalized.
  - **Naming trap.** DAPO calls this *token-level*. [ScaleRL](https://arxiv.org/pdf/2510.13786) calls the same equation *prompt average*, since the $`1/\sum_i|o_i|`$ sits inside the per-prompt expectation and every prompt ends up weighted equally, and reserves *token average* for one mean over every token in the whole batch. With $`P`$ prompts per batch and $`T_p = \sum_i |o_i|`$ for prompt $`p`$, the per-token weight is $`1/(P\,T_p)`$ vs $`1/\sum_p T_p`$. The two agree within a prompt and differ only in how prompts are weighted against each other: token average up-weights prompts whose responses run long. The DAPO code uses verl's `loss_agg_mode="token-mean"`, which is the mean over the whole mini-batch, so **DAPO as written is prompt average and DAPO as run is token average**. ScaleRL's ablation finds prompt average reaches the highest asymptotic reward.
...
### Score centering: an additive correction instead of a reshaped ratio

Source: [Score Centering Stabilizes Off-policy RL](https://arxiv.org/abs/2609.20807) (Marek & Ryabinin, Together AI). Every method above reshapes the IS ratio; this one amends the score values to address sampler-trainer mismatch.

- **Key Idea:** The expected score is zero under *its own* distribution: $`\mathbb{E}_p[s_{y_t}] = \sum_v p_v \nabla_\theta \log p_v = \nabla_\theta \sum_v p_v = \nabla_\theta 1 = 0`$. But under mismatch the token is sampled from the sampler $`q`$ while the score is computed through the trainer $`p`$, so $`\bar s = \mathbb{E}_q[s_{y_t}] = \sum_v q_v \nabla_\theta \log p_v \ne 0`$. Score centering subtracts it back: $`\tilde s_{y_t} = s_{y_t} - \bar s`$.
- **Intuition: drift vs signal.** Apply the covariance identity at a fixed prefix: <div align="center">
  $`\displaystyle \underbrace{\mathbb{E}_q[R\, s_{y_t}]}_{\text{PG update}} = \underbrace{\mathbb{E}_q[R]\,\bar s}_{\text{drift}} + \underbrace{\mathrm{Cov}_q(R,\, s_{y_t})}_{\text{signal}}`$ </div>
  - The covariance term is what we want - it has dependence on *which* token led to *which* reward. The drift term instead is influenced by $`\bar s`$, the negative gradient of the cross-entropy loss with the sampler as teacher — so **vanilla PG under mismatch silently distills the trainer toward the sampler**, scaled by the expected reward at that prefix, which destabilizes training.
  - Centering makes $`\mathbb{E}_q[\tilde s_{y_t}] = 0`$ exactly, killing drift at every prefix, so $`\mathbb{E}_q[R\,\tilde s_{y_t}] = \mathrm{Cov}_q(R, \tilde s_{y_t}) = \mathrm{Cov}_q(R, s_{y_t})`$. **The only remaining gap to the on-policy update is that the covariance is measured under $`q`$ rather than $`p`$** — which is exactly the hole IS fills, hence the composition below.
- **vs IS.** Both cancel drift exactly (see below). But sequence IS correction has high variance over long sequences. And IS no longer cancels exactly once we introduce truncation/masking.
- **Implementation.** Express it as a scalar loss so autograd does the work — no per-vocabulary-token backward pass: <div align="center">
  $`\displaystyle \mathcal{L} = -R\Big(\log p_{y_t} - \sum_v \mathrm{sg}[q_v] \log p_v\Big) \quad\Longrightarrow\quad -\nabla_\theta \mathcal{L} = R\big(s_{y_t} - \textstyle\sum_v q_v s_v\big) = R\,\tilde s_{y_t}`$ </div>
  You only need trainer logprobs over the vocabulary plus the sampler's distribution, and it's position-dependent since both $`p`$ and $`q`$ condition on the prefix.
  - **Top-$`k`$ in practice.** Logging the sampler's full distribution is prohibitive (152K vocab × 32K tokens × 1024 rollouts ≈ 20TB). Log the top-$`k`$ head $`H`$ ($`k=128`$, and $`k=32`$ works) and model the tail with the *trainer's* distribution rescaled by the tail-mass ratio $`\rho = (1 - \sum_H q_v)/(1 - \sum_H p_v)`$. Then, using $`\mathbb{E}_p[s] = 0`$ to eliminate the tail sum, the correction collapses to a head-only sum: <div align="center">
  $`\displaystyle \mathbb{E}_{\hat q}[s_{y_t}] = \sum_{v \in H} q_v s_v + \rho \sum_{v \in T} p_v s_v = \sum_{v \in H} (q_v - \rho\, p_v)\, s_v`$ </div>
  Cost is <1% wall-clock; vLLM and SGLang expose top-$`k`$ logprobs natively.
- **Composing with TIS/MIS.** Since IS reweights the scores and SC centers them, apply SC *on top of* the reweighted scores: with $`w_v = f(r_v)`$, $`r_v = p_v/q_v`$, subtract $`\mathbb{E}_q[w_{y_t} s_{y_t}] = \sum_v q_v w_v s_v`$. Under the top-$`k`$ reconstruction the tail ratio is the constant $`p_v/\hat q_v = 1/\rho`$, so the head-only form survives with $`\alpha = \rho f(1/\rho)`$ in place of $`\rho`$: <div align="center">
  $`\displaystyle \mathcal{L} = -R\Big(\mathrm{sg}[w_{y_t}]\log p_{y_t} - \sum_{v \in H} \mathrm{sg}[q_v w_v - \alpha\, p_v] \log p_v\Big)`$ </div>
  - **Sanity check: exact IS makes SC a no-op.** With $`f(r) = r`$ we get $`\alpha = 1`$ and $`q_v w_v = p_v`$, so the centering term is identically zero — exact IS already has zero expected weighted score, leaving nothing to center. SC only earns its keep once the ratio is truncated or masked, i.e. exactly the regime everyone actually trains in. (Vanilla SC is the $`f = 1`$ case, $`\alpha = \rho`$.) Likewise, with no mismatch ($`p = q`$) the correction is zero.
- **Results** (Qwen3-0.6B on Countdown, Qwen3-30B-A3B-Base on INTELLECT-2 math; all methods share one objective — REINFORCE with group-centered rewards — so each correction is isolated):
  - Under **quantization and synthetic sampler weight noise**, SC alone matches or beats TIS/MIS, and the gap widens with severity; at the largest noise only SC, TIS+SC and MIS+SC train stably at all.
  - Under **staleness**, SC composed with TIS/MIS beats vanilla SC — consistent with the $`\mathrm{Cov}_q`$ vs $`\mathrm{Cov}_p`$ gap above: TIS partially fixes the sampling distribution, SC removes the leftover drift. **Recommendation: compose whenever the trainer can move far from the sampler between syncs.**
...
- **Response-level length bias** from the $`1/|o_i|`$ term. Dr. GRPO replaces it with a constant (the generation budget $`L_{\max}`$ in code), so every token in the batch gets weight $`1/(P\,G\,L_{\max})`$. Nuance vs DAPO: that is the same gradient direction as token average ($`1/\sum_p T_p`$, DAPO as run in verl), differing by one scalar per step: DAPO's normalizer grows with the batch's total token count, so long-response batches take a smaller step, while Dr. GRPO's step grows with response length. It is *not* the same direction as the prompt-average equation written in the [DAPO](#dapo) section: that carries a per-prompt factor $`1/T_p`$, which only reduces to a scalar within a single prompt.
...
- **Resolving the RLOO equivalence.** The group mean violates the EGLP requirement $`B \perp y_i`$, since $`y_i`$ contributes through the $`j = i`$ term. But the resulting "bias" is only a constant rescaling: <div align="center">
  ...
  The [ScaleRL](https://arxiv.org/pdf/2510.13786) paper has interesting ablations that looks into the choice of IS, advantage normalization, loss aggregation, etc. 
