...
- **Caveat on $`k_3`$.** "Unbiased and lower variance" holds when $`p`$ and $`q`$ are close and share support. When tails or supports differ substantially, $`\delta`$ is heavy-tailed and $`k_3`$ can have severe bias and infinite variance. Also, being a good *value* estimator says nothing about being a good *loss*, which is the next section.
...
- **The object.** The RLHF objective is $`\mathcal{J}(\theta) = \mathbb{E}_{x, y \sim \pi_\theta}[r(x, y)] - \beta\, D_{KL}\big(\pi_\theta(\cdot \mid x) \,\|\, \pi_{\mathrm{ref}}(\cdot \mid x)\big)`$: a *reverse* KL, expectation under the current policy, so on-policy samples estimate it directly (forward KL would need samples from $`\pi_{\mathrm{ref}}`$). The KL is on the **full response**: $`\log \pi_\theta(y \mid x) = \sum_t \log \pi_\theta(y_t \mid x, y_{<t})`$, so the per-token KL terms are summed, not averaged. The analysis below is in the bandit view (whole response = one action).
- **The true gradient.** Differentiate $`\mathcal{J}_{KL}(\theta) = \sum_y \pi_\theta(y \mid x) \log \frac{\pi_\theta(y \mid x)}{\pi_{\mathrm{ref}}(y \mid x)}`$ with the product rule, then rewrite as an expectation over the (detached) sampling distribution:
  ```math
  \begin{aligned}
  ...
  &= \mathbb{E}_{y \sim \pi_\theta}\!\left[\underbrace{\log \frac{\pi_\theta(y \mid x)}{\pi_{\mathrm{ref}}(y \mid x)}}_{= -\log\delta = k_1,\ \text{detached}} \nabla_\theta \log \pi_\theta(y \mid x)\right]
  \end{aligned}
  ```
...
From the same paper's appendix, plus the standard hygiene:
